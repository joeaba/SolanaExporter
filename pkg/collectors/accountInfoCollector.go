...
func NewAccountInfoCollector(rpcAddr string) *AccountInfoCollector {
	return &AccountInfoCollector{
		...
		accountOwner: prometheus.NewDesc(
			...
			[]string{"owner", "pubkey", "instance"}, nil),
...
func (c *AccountInfoCollector) mustEmitAccountInfoMetrics(ch chan<- prometheus.Metric, response *rpc.AccountInfo, pubkey string) {
	...
	if response.Value.Data != nil {
		ch <- prometheus.MustNewConstMetric(c.accountData, prometheus.GaugeValue, 0, response.Value.Data[0], pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountExecutable, prometheus.GaugeValue, 0, strconv.FormatBool(response.Value.Executable), pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountLamports, prometheus.GaugeValue, float64(response.Value.Lamports), pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountOwner, prometheus.GaugeValue, 0, response.Value.Owner, pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountRentEpoch, prometheus.GaugeValue, float64(response.Value.RentEpoch), pubkey, "mainnet")
	} else {
		ch <- prometheus.MustNewConstMetric(c.accountData, prometheus.GaugeValue, 0, "the requested account doesn't exist", pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountExecutable, prometheus.GaugeValue, 0, "the requested account doesn't exist", pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountLamports, prometheus.GaugeValue, float64(-1), pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountOwner, prometheus.GaugeValue, 0, "the requested account doesn't exist", pubkey, "mainnet")
		ch <- prometheus.MustNewConstMetric(c.accountRentEpoch, prometheus.GaugeValue, float64(-1), pubkey, "mainnet")
	}
}
...
func (c *AccountInfoCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, pubkey := range keys.Pubkey {
		...
		if err != nil {
			...
			ch <- prometheus.MustNewConstMetric(c.accountData, prometheus.GaugeValue, 0, err.Error(), pubkey, "mainnet")
			ch <- prometheus.MustNewConstMetric(c.accountExecutable, prometheus.GaugeValue, 0, err.Error(), pubkey, "mainnet")
			ch <- prometheus.MustNewConstMetric(c.accountLamports, prometheus.GaugeValue, float64(-1), pubkey, "mainnet")
			ch <- prometheus.MustNewConstMetric(c.accountOwner, prometheus.GaugeValue, 0, err.Error(), pubkey, "mainnet")
			ch <- prometheus.MustNewConstMetric(c.accountRentEpoch, prometheus.GaugeValue, float64(-1), pubkey, "mainnet")
		} else {
