...
func (c *TokenSupplyCollector) mustEmitTokenSupplyMetrics(ch chan<- prometheus.Metric, response *rpc.TokenSupplyInfo, pubkey string) {
	...
	ch <- prometheus.MustNewConstMetric(c.supplyAmount, prometheus.GaugeValue, 0, response.Value.Amount, pubkey, "mainnet")
	ch <- prometheus.MustNewConstMetric(c.supplyAmountDecimals, prometheus.GaugeValue, float64(response.Value.Decimals), pubkey, "mainnet")
	ch <- prometheus.MustNewConstMetric(c.supplyAmountString, prometheus.GaugeValue, 0, response.Value.UiAmountString, pubkey, "mainnet")
}
...
func (c *TokenSupplyCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, pubkey := range keys.Pubkey {
		...
		if err != nil {
			...
			ch <- prometheus.MustNewConstMetric(c.supplyAmount, prometheus.GaugeValue, 0, err.Error(), pubkey, "mainnet")
			ch <- prometheus.MustNewConstMetric(c.supplyAmountDecimals, prometheus.GaugeValue, float64(-1), pubkey, "mainnet")
			ch <- prometheus.MustNewConstMetric(c.supplyAmountString, prometheus.GaugeValue, 0, err.Error(), pubkey, "mainnet")
		} else {
